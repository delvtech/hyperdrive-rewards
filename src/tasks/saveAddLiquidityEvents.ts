...
import { addLiquidityAbiEvent } from "../abi/events";
...
export async function saveAddLiquidityEvents(
    ...
    console.log(`Found ${logs.length} AddLiquidity events`);

    if (!logs) {
        console.log("No AddLiquidity events found");
        return;
    }
    ...
    logs.forEach((log, index) => {
        ...
        console.log(
            `Processing AddLiquidity event at block ${blockNumber} for trader ${provider}`,
        );
    ...
    console.log("Done saving AddLiquidity events.");
}
